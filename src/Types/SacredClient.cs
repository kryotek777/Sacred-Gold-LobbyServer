...
public class SacredClient
{
    ...
    public ServerInfoMessage? ServerInfo { get; private set; }
    ...
    public void ReceivePacket(SacredMsgType type, ReadOnlySpan<byte> payload)
    {
        ...
        switch (type)
        {
            case SacredMsgType.ClientLoginRequest:
                {
                    var request = LoginMessage.Deserialize(payload);
            ...
            case SacredMsgType.ServerLoginRequest:
                {
                    var serverInfo = ServerInfoMessage.Deserialize(payload);
            ...
            case SacredMsgType.ReceiveChatMessage:
                {
                    var message = ChatMessage.Deserialize(payload);
            ...
            case SacredMsgType.PublicDataRequest:
                {
                    var request = PublicDataRequestMessage.Deserialize(payload);
                    OnPublicDataRequest(request);
                }
                break;
            case SacredMsgType.ServerListRequest:
                {
                    var request = RequestServerListMessage.Deserialize(payload);
    ...
    public void OnMessageOfTheDayRequest(ushort id)
    {
        ...
        var motd = new MotdMessage(id, text);
    ...
    private void OnServerListRequest(RequestServerListMessage serverListRequest)
    {
    ...
    private void OnPublicDataRequest(PublicDataRequestMessage request)
    {
    ...
    private void OnReceivePublicData(PublicDataMessage publicData)
    {
    ...
    public void OnClientLoginRequest(LoginMessage loginRequest)
    {
    ...
    private void OnServerLoginRequest(ServerInfoMessage serverInfo)
    {
    ...
    private void OnServerChangePublicInfo(ServerInfoMessage newInfo)
    {
        lock (_lock)
        {
            ServerInfo = ServerInfo! with
            ...
            Log.Info($"GameServer {GetPrintableName()} changed public info {ServerInfo.PlayerCount}/{ServerInfo.MaxPlayers} {ServerInfo.Flags}");
        }
    }
    ...
    private void OnClientChatMessage(ChatMessage message)
    {
        ...
        else    //Normal message
        {
            ...
            Log.Info($"{GetPrintableName()} says: {msg.Text}");
        }
