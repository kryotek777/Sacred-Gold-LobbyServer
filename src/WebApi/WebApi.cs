...
public static partial class WebApi
{
    ...
    public static async Task Run(string url, CancellationToken token)
    {
        try
        {
            ...
            webApp.MapGet("/version", Version).WithTags("Generic");

            // --- [ Accounts ]---
            webApp.MapGet("/accounts/all", AllAccountIds).WithTags("Accounts");
            webApp.MapGet("/accounts/by-name/{name}", GetAccountByName).WithTags("Accounts");
            webApp.MapGet("/accounts/{id}", GetAccountById).WithTags("Accounts");
            webApp.MapGet("/accounts/{id}/characters/{saveId}", GetAccountCharacter).WithTags("Accounts");
            
            webApp.MapPost("/accounts/search/", SearchAccount).WithTags("Accounts");

            // ---[ Statistics ]---
            webApp.MapGet("/statistics", AllStatistics).WithTags("Statistics");
            webApp.MapGet("/statistics/accounts/count", GetRegisteredUsersCount).WithTags("Accounts", "Statistics");
            webApp.MapGet("/statistics/servers", () => LobbyServer.Servers.Count()).WithTags("Statistics");
            webApp.MapGet("/statistics/users", () => LobbyServer.Users.Count()).WithTags("Statistics");
            webApp.MapGet("/statistics/bytes-received", () => Statistics.BytesReceived).WithTags("Statistics");
            webApp.MapGet("/statistics/bytes-sent", () => Statistics.BytesSent).WithTags("Statistics");
            webApp.MapGet("/statistics/packets-received", () => Statistics.PacketsReceived).WithTags("Statistics");
            webApp.MapGet("/statistics/packets-sent", () => Statistics.PacketsSent).WithTags("Statistics");
            webApp.MapGet("/statistics/runtime", () => Statistics.Runtime).WithTags("Statistics");
            webApp.MapGet("/statistics/average-packet-processing-time", () => Statistics.AveragePacketProcessingTime).WithTags("Statistics");
